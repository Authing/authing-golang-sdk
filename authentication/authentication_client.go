...
func NewAuthenticationClient(options *AuthenticationClientOptions) (*AuthenticationClient, error) {
	...
	var checkRes = client.checkAppIdSecret()

	if !checkRes.Data.Valid {
		return nil, errors.New(checkRes.Data.Message)
	}
...
func (client *AuthenticationClient) checkAppIdSecret() *dto.CheckAppIdSecretRespDto {
	body, err := client.SendHttpRequest(
		"/api/v3/check-app-id-secret",
		fasthttp.MethodPost,
		&dto.CheckAppIdSecretDto{
